<%@ page pageEncoding="utf-8" %>
<link rel="shortcut icon" href="${staticPath}/static/img/favicon.ico">
<link href="${staticPath}/static/css/bootstrap.min.css?v=3.3.6" rel="stylesheet">
<link href="${staticPath}/static/css/font-awesome.min.css?v=4.4.0" rel="stylesheet">
<link href="${staticPath}/static/css/animate.css" rel="stylesheet">
<link href="${staticPath}/static/css/style.css?v=4.1.0" rel="stylesheet">
<link href="${staticPath}/static/css/plugins/summernote/summernote.css" rel="stylesheet">
<link href="${staticPath}/static/css/plugins/summernote/summernote-bs3.css" rel="stylesheet">
<link href="${staticPath}/static/css/plugins/cropper/cropper.min.css" rel="stylesheet">
<link href="${staticPath}/static/css/font-awesome.css?v=4.4.0" rel="stylesheet">
<link rel="stylesheet" href="${staticPath}/static/js/plugins/qqface/css/qqFace.css">
<!-- Latest compiled and minified CSS -->
<link rel="stylesheet" href="${staticPath}/static/css/plugins/bootstrap-table/bootstrap-table.min.css">

<!-- 全局js -->
<script src="${staticPath}/static/js/jquery.min.js?v=2.1.4"></script>
<script src="${staticPath}/static/js/jquery-migrate-1.4.1.js"></script>
<script src="${staticPath}/static/js/bootstrap.min.js?v=3.3.6"></script>
<script src="//cdnjs.cloudflare.com/ajax/libs/bootstrap-table/1.11.1/bootstrap-table.min.js"></script>
<script src="${staticPath}/static/js/plugins/metisMenu/jquery.metisMenu.js"></script>
<script src="${staticPath}/static/js/plugins/slimscroll/jquery.slimscroll.min.js"></script>
<script src="${staticPath}/static/js/plugins/layer/layer.min.js"></script>
<script src="${staticPath}/static/js/plugins/layer/laypage/laypage.js"></script>
<script src="${staticPath}/static/js/plugins/qqface/jquery.qqFace.js"></script>
<script src="${staticPath}/static/js/plugins/bootstrap-table/locale/bootstrap-table-zh-CN.min.js"></script>
<!-- layerDate plugin javascript -->
<script src="${staticPath}/static/js/plugins/layer/laydate/laydate.js"></script>
<!-- SUMMERNOTE -->
<script src="${staticPath}/static/js/plugins/summernote/summernote.min.js"></script>
<script src="${staticPath}/static/js/plugins/summernote/summernote-zh-CN.js"></script>
<!-- Peity -->
<script src="${staticPath}/static/js/plugins/peity/jquery.peity.js"></script>
<script src="${staticPath}/static/js/demo/peity-demo.js"></script>
<!-- 自定义js -->
<script src="${staticPath}/static/js/hAdmin.js?v=4.1.0"></script>
<script src="${staticPath}/static/js/index.js"></script>
<script src="${staticPath}/static/js/content.js?v=1.0.0"></script>
<!-- Image cropper -->
<script src="${staticPath}/static/js/plugins/cropper/cropper.min.js"></script>
<!-- 第三方插件 -->
<script src="${staticPath}/static/js/plugins/pace/pace.min.js"></script>
<script src="${staticPath}/static/js/plugins/sockjs/sockjs.min.js"></script>
