...
import com.forumShiro.listener.Online;
import com.forumShiro.mapper.*;
import com.forumShiro.model.*;
...
public class UserController {
        ...
        if (subject.getPrincipal() != null) {
            ...
            if (user.getUstate() == 0) {
                ...
                if (user.getUtype() == 1) {
                    ...
                    System.out.println("登录成功：" + user.getUnickname()+","+user.getUtype());
                    return R.ok(2,"manage");
                }
            } else {
                /////用户被封了
                return R.error(-1, user.getUstate().toString());
            }
        } else {
            return R.error("err");
        }
    ...
    public void alterpassword(HttpServletRequest request,
        ...
        PrintWriter pw = null;
        if (!ShiroUtils.sha256(beginp, user.getUsalt()).equals(password) && beginp.equals(endp)) {
            pw = response.getWriter();
            ...
            if (!sendyzm.equals(yzm)) {
                ...
                user.setUpassword(ShiroUtils.sha256(beginp, user.getUsalt()));
                userMapper.updateById(user);
                //记录
    ...
    public void forgetpassword(@RequestParam("email") String email,
        ...
        if (sendyzm != null && sendyzm.length() > 0) {
            ...
            if (sendyzm.equals(yzm)) {
                ...
                user.setUpassword(ShiroUtils.sha256(newpassword, user.getUsalt()));
            ...
            if (sendyzm.equals(yzm)) {
                ...
                if (matcher1.matches() && matcher2.matches() && unickname.trim().length() > 0) {
                    ...
                    if (user == null || user.size() == 0) {
                        ...
                        newUser.setUsalt(RandomStringUtils.randomAlphanumeric(25));
                        newUser.setUpassword(ShiroUtils.sha256(upassword, newUser.getUsalt()));
